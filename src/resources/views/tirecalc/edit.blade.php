        ...
        <div x-data='taxCalculator(@json($tirecalc))' class=" max-w-2xl p-6 bg-white rounded shadow space-y-8">
            ...
            <form method="POST" :action="action" x-data="{ action: '{{ route('tirecalc.createPdf') }}' }" id="pdf-form">
                ...
                <div class="p-4 border rounded space-y-4">
                    ...
                    <div x-data="laborSettings()" x-init="loadFromCookie()" class="space-y-4">
                        <!-- 保存チェックボックス -->
                        <label class="inline-flex items-center">
                            <input type="checkbox" x-model="saveToCookie" @change="handleCookieToggle" class="mr-2">
                            工賃設定を保存する
                        </label>
                    ...
                    <script>
                        function laborSettings() {
                            return {
                                ...
                                get laborSubtotal() {
                                    let sum = 0;
                                    this.laborItems.forEach(item => {
                                        sum += (item.price || 0) * (item.quantity || 1);
                                    });
                                    if (this.laborTaxMode === 'excluding') {
                                ...
                                saveToCookieStorage() {
                                    const value = {
                                        laborTaxMode: this.laborTaxMode,
                                        laborItems: this.laborItems
                                    };
                                    document.cookie = `laborSettings=${encodeURIComponent(JSON.stringify(value))}; max-age=${60 * 60 * 24 * 365}; path=/`;
                                },

                                loadFromCookie() {
                                    const match = document.cookie.match(/(?:^|; )laborSettings=([^;]*)/);
                                    if (match) {
                                        try {
                                            const data = JSON.parse(decodeURIComponent(match[1]));
                                            this.laborTaxMode = data.laborTaxMode || 'excluding';
                                            this.laborItems = data.laborItems || [];
                                            this.saveToCookie = true;
                                        } catch (e) {
                                            console.error('クッキー読み込みエラー', e);
                                        }
                                    }
                                },

                                clearCookie() {
                                    document.cookie = `laborSettings=; max-age=0; path=/`;
                                }
                            }
                        }
                    </script>
