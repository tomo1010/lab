        ...
        $nextTick(() => {
            ...
            <div class="bg-white p-8 border border-gray-300 rounded-md shadow-md text-sm">
                ...
                <div class="text-right text-xs text-gray-600 mb-2">
                    発行日：
                    <input type="date" name="date" value="{{ old('date', $invoice->date) }}" class="border rounded px-2 py-1 text-xs" required>
                </div>
                ...
                <div class="text-right text-xs text-gray-600 mb-4">
                    ...
                    <select name="page_count" class="border rounded px-2 py-1 text-xs">
                        @for ($i = 1; $i <= 10; $i++)
                            <option value="{{ $i }}" {{ (old('page_count', $invoice->page_count) == $i) ? 'selected' : '' }}>
                            {{ $i }}
                            </option>
                            @endfor
                    </select>
                ...
                <div class="mb-6">
                    ...
                    <div class="flex gap-2">
                        <input type="text" name="client" value="{{ old('client', $invoice->client) }}" class="w-full border rounded px-2 py-1">
                        <select name="to_suffix" value="{{ old('to_suffix', $invoice->to_suffix) }}" class="border rounded px-2 py-1 text-sm">
                ...
                <div class="mb-6">
                    ...
                    <input type="text" name="client_address" value="{{ old('client_address', $invoice->client_address) }}" class="w-full border rounded px-2 py-1" placeholder="〒を全角入力→変換">
                </div>

                <div class="mb-6"
                    x-data="{
                        prices: [
                            {{ old('price_1', $invoice->price_1 ?? 0) }},
                            {{ old('price_2', $invoice->price_2 ?? 0) }},
                            {{ old('price_3', $invoice->price_3 ?? 0) }},
                            {{ old('price_4', $invoice->price_4 ?? 0) }},
                            {{ old('price_5', $invoice->price_5 ?? 0) }}
                        ],
                        get total() {
                    ...
                    @for ($i = 1; $i <= 5; $i++)
                        ...
                        <input
                            ...
                            placeholder="項目"
                            value="{{ old('item_' . $i, $invoice->{'item_' . $i} ?? '') }}">
                        <input
                            name="price_{{ $i }}"
                            type="number"
            ...
            <div class="mb-6">
                ...
                <textarea name="message" value="{{ old('message', $invoice->message) }}" class="w-full h-24 border rounded px-2 py-1 text-sm"></textarea>
            </div>
            ...
            <div class="submit-btn flex gap-4 justify-center mt-6">
                ...
                <button
                    type="button"
                    @click="setAction('{{ route('pdf.generatePdf') }}', 'POST')"
                ...
                <button
                    type="button"
                    @click="setAction('{{ route('invoice.update', $invoice->id) }}', 'PUT')"
                    class="bg-green-400 text-white rounded px-6 py-2 hover:bg-green-700">
                    更新
                </button>
                @endauth
            </div>
